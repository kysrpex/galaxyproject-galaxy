...
class URIModelExportStore(abc.ABC, DirectoryModelExportStore):
    """
    Handle exports to file sources, where data can be retrieved later using a URI.
    """
    ...
    def _generate_output_file(self) -> None:
        ...
        Produce an output file and save it to `self.out_file`. A common pattern for this method is creating an archive
        out of `self.export_directory`.
...
class BcoModelExportStore(URIModelExportStore, WorkflowInvocationOnlyExportStore):
...
class ROCrateArchiveModelExportStore(URIModelExportStore, WriteCrates):
...
class TarModelExportStore(URIModelExportStore):
...
class BagArchiveModelExportStore(URIModelExportStore, BagDirectoryModelExportStore):
...
def get_export_store_factory(
    ...
    user_context=None,
) -> Callable[[StrPath], URIModelExportStore]:
    export_store_class: Type[URIModelExportStore]
